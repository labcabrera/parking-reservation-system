<!--
  ...
  Added sections:
    - I.   Hexagonal Architecture & Clean Architecture (new)
    - II.  Domain-Driven Design (new)
    - III. Test-First Discipline (new)
    - IV.  Concurrency & Distributed Data Consistency (new)
    - V.   Performance & Resilience (new)
    - VI.  Observability (new)
    - VII. Language Discipline (new)
...
Test-Driven Development (TDD) is REQUIRED for all domain and application code.

- The Red-Green-Refactor cycle MUST be followed: write a failing test first, make it pass
  with the minimal implementation, then refactor.
- The domain layer MUST achieve ≥ 90% unit test line coverage. Coverage MUST be verified
  in CI and failures MUST block merge.
- Adapter integration tests MUST use Testcontainers or equivalent test doubles. Production
  infrastructure MUST NOT be accessed in CI unit/integration test stages.
- Tests MUST be written and confirmed failing before the corresponding production code is
  reviewed or merged.
- React frontend components MUST be tested with React Testing Library. Coverage target
  is ≥ 80% for components containing business logic.

### IV. Concurrency & Distributed Data Consistency
...
and distributed lock support. Schema migrations MUST be managed via Flyway.

**Messaging**: Apache Kafka for domain event streaming and SAGA coordination between
bounded contexts.

**Build & CI**: Maven (backend). npm / Vite (frontend). GitHub Actions for CI/CD. All
...
4. **TDD** is enforced at every implementation step (Principle III).
5. **Code review** MUST have a minimum of 1 approval. The reviewer MUST verify
   constitution compliance, test coverage targets, and the absence of cross-layer
   dependency violations (enforced by ArchUnit).
...
semantic versioning rules below. Amendments that affect Principles I, II, or III
(architecture, DDD, or TDD discipline) require explicit team consensus before merging.
