...
public interface InventorySlotJpaRepository extends JpaRepository<InventorySlotJpaEntity, UUID> {
    ...
    /**
     * Optimistic atomic hold: increments {@code reserved} only when capacity remains AND
     * the row hasn't changed since we read it. Returns the number of affected rows (1 =
     * success, 0 = stale version or no capacity).
     */
     ...
     * availability search to compute the minimum free spots across the requested range in
     * a single round-trip. Facilities absent from the result have no recorded slots in
     * range and are fully available. Each row:
     * {@code [UUID facilityId, Integer maxReserved]}.
     */
    @Query("""
