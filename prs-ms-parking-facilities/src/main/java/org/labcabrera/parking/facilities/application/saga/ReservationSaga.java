package org.labcabrera.parking.facilities.application.saga;
...
import org.labcabrera.parking.facilities.application.service.InventoryHoldService;
import org.labcabrera.parking.facilities.application.service.InventoryHoldService.HoldResult;
 ...
 * <ol>
 * <li>On {@link ReservationStartedEvent} → attempt inventory hold via optimistic locking
 * on every 30-min slot in range. On any per-slot failure, all previously held slots are
 * compensated and the reservation is marked FAILED.</li>
 * <li>On success → compute estimated price (stub; pricing-service async wiring TODO) and
 * dispatch {@link MarkReservationHeldCommand}. Schedule a deadline at {@code expiresAt}
 * that fires {@link ExpireReservationCommand}.</li>
 * <li>On confirm / cancel / expire → release inventory (for cancel/expire) and end the
 * saga.</li>
 * </ol>
 */
@Saga
...
public class ReservationSaga {
    ...
    /**
     * TODO replace with async pricing-service call (Kafka request/response). Uses
     * facility's estimatedDailyPrice prorated by duration as a placeholder.
     */
