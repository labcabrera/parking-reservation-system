...
const PAYMENT_SERVICE_URL =
  (import.meta.env.VITE_PAYMENT_SERVICE_URL as string | undefined)?.replace(
    /\/$/,
    "",
  ) ?? "";
...
export async function capturePaymentAttempt(
  ...
  const response = await fetch(
    `${PAYMENT_ATTEMPTS_URL}/${encodeURIComponent(attemptId)}/pay`,
    {
      body: JSON.stringify({ callbackUrl, orderId, status: "SUCCESS" }),
      headers: { "Content-Type": "application/json" },
      method: "POST",
    },
  );
...
async function buildErrorMessage(response: Response) {
  ...
  return details
    ? `La pasarela mock rechazó el pago: ${response.status} ${response.statusText}. ${details}`
    : `La pasarela mock rechazó el pago: ${response.status} ${response.statusText}`;
}
